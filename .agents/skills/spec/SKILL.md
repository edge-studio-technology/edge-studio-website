...
4. Run the `domain-check` skill if the change touches sensitive areas defined in `.agents/rules/domain-rules.md`.
5. Explore relevant code with `Glob`, `Grep`, and `ReadFile` to understand existing patterns.
...
After approval, use `/skill:feature` or `/skill:fix` to implement from the spec.
