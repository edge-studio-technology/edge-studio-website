...
> Imported from https://github.com/Jakubantalik/transitions.dev/tree/main/skills/transitions-dev (no license file declared upstream — snippets are explicitly offered as copy-ready by the source project).
...
| --- | --- | --- |
| **Card resize** | Tween a container's width or height when its layout state changes. | [01-card-resize.md](references/01-card-resize.md) |
| **Number pop-in** | Re-enter each digit with a blurred slide when a number updates. | [02-number-pop-in.md](references/02-number-pop-in.md) |
| **Notification badge** | Slide a small badge onto a trigger and pop the dot. | [03-notification-badge.md](references/03-notification-badge.md) |
| **Text states swap** | Swap text in place with a blurred up-and-down transition. | [04-text-states-swap.md](references/04-text-states-swap.md) |
| **Menu dropdown** | Open an origin-aware dropdown that grows from its trigger. | [05-menu-dropdown.md](references/05-menu-dropdown.md) |
| **Modal open / close** | Scale-up modal dialog with a softer scale-down on close. | [06-modal.md](references/06-modal.md) |
| **Panel reveal** | Slide a panel into a region with a cross-blur. | [07-panel-reveal.md](references/07-panel-reveal.md) |
| **Page side-by-side** | Slide between two side-by-side pages (list ↔ detail, step 1 ↔ step 2). | [08-page-side-by-side.md](references/08-page-side-by-side.md) |
| **Icon swap** | Cross-fade two icons in the same slot with blur and scale. | [09-icon-swap.md](references/09-icon-swap.md) |
| **Success check** | Compose fade + rotate + Y-bob + path stroke-draw to celebrate a completed action. | [10-success-check.md](references/10-success-check.md) |
| **Avatar group hover** | Distance-falloff lift on a row of items with a bouncy spring on return. | [11-avatar-group-hover.md](references/11-avatar-group-hover.md) |
| **Error state shake** | Per-segment cubic-bezier shake with auto-reverting border + message. | [12-error-state-shake.md](references/12-error-state-shake.md) |
| **Input clear with dissolve** | Fly-out + per-word streak when a text field is cleared. | [13-input-clear-dissolve.md](references/13-input-clear-dissolve.md) |
| **Skeleton loader and reveal** | Pulse a placeholder, then cross-fade + cross-blur to the loaded content. | [14-skeleton-reveal.md](references/14-skeleton-reveal.md) |
| **Shimmer text** | Sweep a highlight band across muted text on a loop (pure CSS). | [15-shimmer-text.md](references/15-shimmer-text.md) |
| **Tabs sliding** | Slide the active pill between tabs in a segmented control. | [16-tabs-sliding.md](references/16-tabs-sliding.md) |
| **Tooltip open/close** | Delayed fade+scale in, instant out (pure CSS). | [17-tooltip.md](references/17-tooltip.md) |
| **Texts reveal** | Staggered blurred rise for stacked text lines, quiet fade out. | [18-texts-reveal.md](references/18-texts-reveal.md) |
| **Card hover tilt** | Tilt a card in 3D toward the pointer with a cursor-tracked glare. | [19-card-tilt.md](references/19-card-tilt.md) |
| **Plus to menu morph** | Morph a circular trigger into the menu / panel it opens. | [20-plus-menu-morph.md](references/20-plus-menu-morph.md) |
| **Accordion expand** | Grow / shrink a panel via grid-rows with a chevron flip. | [21-accordion.md](references/21-accordion.md) |
| **Toast open / close** | Rise a toast from below with fade + cross-blur, slower in than out. | [22-toast.md](references/22-toast.md) |
| **Like button** | Fill a heart with a pop + particle burst on like. | [23-like-button.md](references/23-like-button.md) |
| **Learn more hover** | Slide the chevron and spread its arms into an arrow on hover. | [24-learn-more-hover.md](references/24-learn-more-hover.md) |
| **Checkbox check** | Fill the box, then stroke-draw the checkmark. | [25-checkbox-check.md](references/25-checkbox-check.md) |
| **Spinning counter** | Spin slot-machine digit reels with vertical motion blur. | [26-spinning-counter.md](references/26-spinning-counter.md) |
| **Toggle** | Travel the switch thumb with a double-bounce overshoot. | [27-toggle.md](references/27-toggle.md) |
...
The shared motion scale behind the twenty-one transitions — the same tokens the [transitions.dev](https://transitions.dev) Motion tokens tab exposes. They ship at the top of [`_root.css`](references/_root.css), so once it's imported you can reference any of them as `var(--…)` (e.g. `transition: transform var(--duration-fast) var(--ease-smooth-out)`).
...
Copy [`_root.css`](references/_root.css) into your project **once** and import it (or paste its `:root` block into your global stylesheet). It leads with the shared **motion-token scale** (`--duration-*`, `--ease-*`, `--distance-*`, `--scale-*`, `--blur-*` — see `## Motion tokens`), followed by the semantic tunable variables for **all twenty-one** transitions. Every snippet reads from these names — `--resize-*`, `--badge-*`, `--dropdown-*`, `--clear-*`, `--shimmer-*`, `--tabs-*`, `--tt-*`, `--stagger-*`, `--tilt-*`, `--morph-*`, `--acc-*`, and the rest.
...
- [01-card-resize.md](references/01-card-resize.md) — Card resize
- [02-number-pop-in.md](references/02-number-pop-in.md) — Number pop-in
- [03-notification-badge.md](references/03-notification-badge.md) — Notification badge
- [04-text-states-swap.md](references/04-text-states-swap.md) — Text states swap
- [05-menu-dropdown.md](references/05-menu-dropdown.md) — Menu dropdown
- [06-modal.md](references/06-modal.md) — Modal open / close
- [07-panel-reveal.md](references/07-panel-reveal.md) — Panel reveal
- [08-page-side-by-side.md](references/08-page-side-by-side.md) — Page side-by-side
- [09-icon-swap.md](references/09-icon-swap.md) — Icon swap
- [10-success-check.md](references/10-success-check.md) — Success check
- [11-avatar-group-hover.md](references/11-avatar-group-hover.md) — Avatar group hover
- [12-error-state-shake.md](references/12-error-state-shake.md) — Error state shake
- [13-input-clear-dissolve.md](references/13-input-clear-dissolve.md) — Input clear with dissolve
- [14-skeleton-reveal.md](references/14-skeleton-reveal.md) — Skeleton loader and reveal
- [15-shimmer-text.md](references/15-shimmer-text.md) — Shimmer text
- [16-tabs-sliding.md](references/16-tabs-sliding.md) — Tabs sliding
- [17-tooltip.md](references/17-tooltip.md) — Tooltip open/close
- [18-texts-reveal.md](references/18-texts-reveal.md) — Texts reveal
- [19-card-tilt.md](references/19-card-tilt.md) — Card hover tilt
- [20-plus-menu-morph.md](references/20-plus-menu-morph.md) — Plus to menu morph
- [21-accordion.md](references/21-accordion.md) — Accordion expand
- [22-toast.md](references/22-toast.md) — Toast open / close
- [23-like-button.md](references/23-like-button.md) — Like button
- [24-learn-more-hover.md](references/24-learn-more-hover.md) — Learn more hover
- [25-checkbox-check.md](references/25-checkbox-check.md) — Checkbox check
- [26-spinning-counter.md](references/26-spinning-counter.md) — Spinning counter
- [27-toggle.md](references/27-toggle.md) — Toggle
- [_root.css](references/_root.css) — the universal install block on its own, ready to import directly.
