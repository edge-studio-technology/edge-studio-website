...
Use when the user wants to define or update multi-milestone direction for the project — not for a single change (that's `/skill:plan`).
...
4. When a plan is created via `/skill:plan` for work under a milestone, link the plan file path next to that milestone.
